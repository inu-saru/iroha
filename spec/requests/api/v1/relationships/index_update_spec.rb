...
RSpec.describe 'PUT /api/v1/spaces/:space_id/relationships/:relationship_id' do
  ...
  context '有効なTokenがある場合' do
    before do
      ...
      params[:relationship] = { positions: [99, 999] }
    end

    context '指定したspaceがownerであるrelationship.idの場合' do
      it '正しく更新したrelationship情報が返ること' do
        subject
        expect(response).to have_http_status(:ok)
        updated_attribute = json_attributes(RelationshipResource.new(relationship1).serialize).merge(params[:relationship])
        expect(json_response).to eq updated_attribute
      end

      it 'positions以外のparameterを送っても更新されないこと' do
        params[:relationship][:language_type] = 'ja'

        subject
        expect(response).to have_http_status(:ok)
        expect(json_response[:language_type]).to eq 'en'
      end
    end
